window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function failCopy(d) {
			...
			if (d.reason === "write-failed" && d.rolledBack === false) return `${base}（回滚未成功，请手动用 backups 目录恢复）`;
			return base;
		}
		function PluginManagementView({ close }) {
			...
			const batch = async (url, payload, label) => {
				try {
			...
			const applyPreset = (mode) => batch("/plugin-management/api/preset", { mode }, mode === "lean" ? "精简模式" : "全量模式");
			const applyCategory = (category, enabled) => batch("/plugin-management/api/category", {
				category,
				enabled
			}, `${enabled ? "启用" : "停用"}整组`);
					...
					groups.map(([cat, list]) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", {
								...
								children: list.some((it) => !it.enabled && !it.isCore) ? "整组启用" : "整组停用"
							})]
